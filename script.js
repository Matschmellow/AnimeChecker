...
// Simple in-memory cache: slug -> { seasons, timestamp }
const episodeCache = {};
...
// ── Improved similarity: Jaccard on bigrams (more accurate than char-set) ──
...
// ── Best slug picker: uses both slug similarity AND returned title similarity ──
function pickBestSlug(searchResults, queryName) {
  ...
  for (const r of searchResults) {
    ...
    // If backend returned a title, factor that in too
...
async function addAnime() {
  ...
  try {
    ...
    if (searchResp.results && searchResp.results.length > 0) {
      // Try with jpTitle too for better results
...
// ── Auto-update: re-check episode count for verified, non-film seasons ──
async function checkForNewEpisodes(animeId, tabNumber) {
  ...
  if (cached && (Date.now() - cached.timestamp) < CACHE_TTL_MS) return; // still fresh
...
function switchTab(animeId, tabNumber) {
  ...
  // Try auto-update when switching to a tab
...
function renderList() {
  ...
  sorted.forEach(anime => {
    ...
    const curTab = anime.activeTab || anime.seasons[0]?.number || 1;
    ...
    let nächsteFolge = 1;
    while (anime.watchedEpisodes.includes(`s${curTab}e${nächsteFolge}`) && nächsteFolge <= maxBoxen) nächsteFolge++;
    ...
    let streamUrl;
    if (isFilmType) {
      streamUrl = `https://aniworld.to/anime/stream/${anime.slug}/filme/film-${nächsteFolge}`;
    } else {
      const aniWorldSeason = seasonData.aniWorldSeason || seasonData.number;
      streamUrl = `https://aniworld.to/anime/stream/${anime.slug}/staffel-${aniWorldSeason}/episode-${nächsteFolge}`;
    }
    ...
    if (isAllFinished) {
      ...
      const btnText = isFilmType ? `🎬 Film ${nächsteFolge} schauen` : `St. ${seasonData.aniWorldSeason || curTab} Folge ${nächsteFolge} schauen`;
      actionButtonHtml = `<a href="${streamUrl}" target="_blank" class="stream-link" onclick="watchEpisodeAuto(${anime.id},${curTab},${nächsteFolge})">${btnText}</a>`;
    }
    ...
    .then(data => {
      if (data.data) {
        ...
        processed.forEach(item => {
          if (!seenSlugs.has(item.slug)) { seenSlugs.add(item.slug); uniqueRecs.push(item); }
        });
...
// ── Startup: auto-update all ongoing animes ──
